...
class TitleState extends MusicBeatState
{
	...
	override public function create():Void
	{
		...
		if(ClientPrefs.checkForUpdates && !closedState) {
			...
			http.onData = function (data:String)
			{
				...
				var curVersion:String = MainMenuState.VPsychVer.trim();
	...
	function startIntro()
	{
		...
		var path = #if android SUtil.getStorageDirectory() + #end "mods/" + Paths.currentModDirectory + "/images/titleEnter.png";
		//trace(path, FileSystem.exists(path));
		if (!FileSystem.exists(path)){
			path = #if android SUtil.getStorageDirectory() + #end "mods/images/titleEnter.png";
		}
		//trace(path, FileSystem.exists(path));
		if (!FileSystem.exists(path)){
			path = #if android SUtil.getStorageDirectory() + #end "assets/images/titleEnter.png";
		}
		//trace(path, FileSystem.exists(path));
	...
	override function beatHit()
	{
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				...
				case 2:
					deleteCoolText();
					createCoolText(['A Custom FNF Psych Engine by'], 15);
				// credTextShit.visible = true;
				case 4:
					addMoreText('Huy1234TH', 15);
				// credTextShit.text += '\npresent...';
